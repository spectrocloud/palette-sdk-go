...
func (h *V1Client) GetMachineListAks(configUID string, machinePoolName string, ClusterContext string) ([]*models.V1AzureMachine, error) {
	client, err := h.GetClusterClient()
	if err != nil {
		return nil, err
	}
...
func (h *V1Client) GetMachinesItemsActionsAks(configUID string, machinePoolName string, ClusterContext string) (map[string]string, error) {
	mpList, err := h.GetMachineListAks(configUID, machinePoolName, ClusterContext)
