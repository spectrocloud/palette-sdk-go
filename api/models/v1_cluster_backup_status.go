		...
		if m.ClusterBackupStatuses[i] != nil {
			if err := m.ClusterBackupStatuses[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					...
					return ce.ValidateName("clusterBackupStatuses" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
// ContextValidate validate this v1 cluster backup status based on the context it is used
func (m *V1ClusterBackupStatus) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	if err := m.contextValidateClusterBackupStatuses(ctx, formats); err != nil {
		res = append(res, err)
	}
...
func (m *V1ClusterBackupStatus) contextValidateClusterBackupStatuses(ctx context.Context, formats strfmt.Registry) error {
	...
	for i := 0; i < len(m.ClusterBackupStatuses); i++ {
		...
		if m.ClusterBackupStatuses[i] != nil {
			...
			if err := m.ClusterBackupStatuses[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("clusterBackupStatuses" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("clusterBackupStatuses" + "." + strconv.Itoa(i))
